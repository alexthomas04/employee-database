...
int main(int argc, char *argv[]) { 
  ...
  if((newfile?create_db_header(&header):validate_db_header(database_fd,&header))==STATUS_ERROR){
  ...
  if(addEmployee != NULL){
    employees = realloc(employees, sizeof(struct employee_t)*(header->count+1));
    if(employees == NULL){
